...
public class PutObjectMetaTask extends StateTask<ReqPutObjectMeta> {
    private static Logger LOG = Logger.getLogger(PutObjectMetaTask.class);
    ...
    public PutObjectMetaTask(ReqPutObjectMeta req,
            HttpServletResponse servletRsp, HttpServletRequest servletReq) {
        super(String.format("%s_%d", "PutObjectMetaTask",
    ...
    public void writeMeta() throws Exception {
        PebbleDB.updateMeta(pebble, req.getMetas());
        LOG.info("Put Pebble Meta OK: pebble = " + pebble.getPebbleID());
        complete();
    }
